...
export default function UserDropdown() {
  ...
  function handleSignOut() {
    // Clear auth flags and tokens (cookies + localStorage for compatibility)
    try {
      localStorage.removeItem("isLoggedIn");
      localStorage.removeItem("fd_token");
      localStorage.removeItem("fd_token_expiry");
    } catch (e) {
      // ignore silently if localStorage access fails
    }

    // Clear cookies
    try {
      clearCookie("fd_token");
      clearCookie("fd_token_expiry");
    } catch (e) {
      // ignore
    }

    // close dropdown and navigate to signin
    closeDropdown();
    navigate("/signin");
  }
